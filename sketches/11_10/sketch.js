...
function draw(){
  ...
  for(let i=0;i< VERT_NUM; i++){
    ...
    const vertRatio = (i / VERT_NUM) % 1;
    let x = lerp(x1(frameRatio - DELAY_FACTOR * vertRatio*10), x2(frameRatio - DELAY_FACTOR * (20 - vertRatio)), vertRatio);
    let y = lerp(y1(frameRatio - DELAY_FACTOR * vertRatio), y2(frameRatio - DELAY_FACTOR * (50 - vertRatio)), vertRatio);
    vertex(x, y - 50);
  ...
  for(let i=0;i< VERT_NUM; i++){
    const vertRatio = (i / VERT_NUM) % 1;
    let x = lerp(x1(frameRatio - DELAY_FACTOR * vertRatio*5), x2(frameRatio - DELAY_FACTOR * (20 - vertRatio)), vertRatio);
    let y = lerp(y1(frameRatio - DELAY_FACTOR * vertRatio*2), y2(frameRatio - DELAY_FACTOR * (10 - vertRatio)), vertRatio);
    vertex(x, y);
